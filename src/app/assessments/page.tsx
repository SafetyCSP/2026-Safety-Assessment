...
export default function AssessmentsPage() {
    ...
    const { getSavedAssessments, loadAssessment, deleteAssessment, totalQuestionCount } = useAssessment();

    const assessments = useMemo(() => getSavedAssessments(), [getSavedAssessments]);
    ...
    const getProgress = (assessment: SavedAssessment) => {
        const answered = Object.values(assessment.answers).filter((answer) => answer.status && answer.status !== 'Unanswered').length;
        const percentage = totalQuestionCount > 0 ? Math.min(100, Math.round((answered / totalQuestionCount) * 100)) : 0;
        ...
        <div className="min-h-screen bg-muted/20">
            <div className="max-w-5xl mx-auto px-4 py-12 sm:px-6 lg:px-8">
                <div className="text-center mb-10 space-y-4">
                    ...
                    <h1 className="text-3xl font-bold tracking-tight text-foreground sm:text-4xl">My Assessments</h1>
                    <p className="text-muted-foreground max-w-xl mx-auto">View, resume, and manage your saved assessments.</p>
                </div>

                <div className="flex justify-center mb-8">
                    <Link href="/" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-primary text-primary-foreground font-semibold shadow-lg hover:opacity-90 transition-all">
                ...
                {assessments.length === 0 ? (
                    <div className="text-center py-16 bg-card border rounded-xl">
                        ...
                        <p className="text-sm text-muted-foreground mt-1">Start a new assessment from the home page.</p>
                    </div>
                ) : (
                    <div className="space-y-4">
                        {assessments.map((assessment) => {
                            ...
                            return (
                                <div key={assessment.id} className="bg-card border rounded-xl p-5 hover:border-primary/30 transition-colors">
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                        ...
                                        <div className="flex items-center gap-2 shrink-0">
                                            {isCompleted ? (
                                                <button onClick={() => handleViewReport(assessment.id)} className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-lg bg-primary text-primary-foreground hover:opacity-90 transition-all">
                                                    <Eye size={14} /> View Report
                                                </button>
                                            ) : (
                                                <button onClick={() => handleResume(assessment.id)} className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-lg bg-primary text-primary-foreground hover:opacity-90 transition-all">
                                                    <Play size={14} /> Resume
                                                </button>
                                            )}
                                            <button
